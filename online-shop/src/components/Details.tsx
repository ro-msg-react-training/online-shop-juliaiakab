...
function Details({match}: RouteComponentProps<TParams>) {
    ...
    const productDetails = selectedCategory?.details;
    const classes = useStyles();
   
    return (
        <div>
            <div className='SameLine'>
            <h1 className="Title2"><b>Product:&nbsp;&nbsp;{productName}</b></h1>
            <Button variant="contained" color="secondary" className={classes.button} startIcon={<AddShoppingCartIcon/>}>
                Add to cart
            </Button>
        </div>
        <Grid container className='Custom-grid' spacing={1}>
            <Grid className="Custom-item" item xs={6}>
                Name:
            </Grid>
            <Grid className="Custom-item" item xs={6}>
                {productName}
